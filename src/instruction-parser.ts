...
function extractInstructionOverrideFields(
  ...
  // Validate and extract description (the only overridable field)
 ...
 *
 * Returns a discriminated union: `{ ok: true, instruction, warnings }` on success,
...
export function parseInstructionContent(content: string): ParseInstructionResult {
  ...
  const nameError = validateName(data.name, 'setup-guide');
