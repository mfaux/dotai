...
export class WellKnownProvider implements HostProvider {
  ...
  private isValidSkillEntry(entry: unknown): entry is WellKnownSkillEntry {
    return isValidSkillEntry(entry);
  }
    ...
    try {
      ...
      const filePromises = otherFiles.map(async (filePath) => {
        try {
          ...
          const fileResponse = await fetch(fileUrl, { signal: AbortSignal.timeout(10_000) });
          if (fileResponse.ok) {
...
/**
 * Validate a skill entry from the index.
 * Exported for testing; used internally by WellKnownProvider.
 */
export function isValidSkillEntry(entry: unknown): entry is WellKnownSkillEntry {
