...
>>>>>>> feature-branch
    ...
    it('throws LockVersionError for future versions', async () => {
      ...
      try {
        ...
        await expect(readLocalLock(dir)).rejects.toThrow(LockVersionError);
        await expect(readLocalLock(dir)).rejects.toThrow('version 99');
        await expect(readLocalLock(dir)).rejects.toThrow('Please upgrade dotai');
      } finally {
        await rm(dir, { recursive: true, force: true });
      }
    });
    ...
    it('throws LockVersionError with correct properties for future versions', async () => {
      ...
      try {
        await writeFile(
          join(dir, 'skills-lock.json'),
          JSON.stringify({ version: 5, skills: {} }),
          'utf-8'
        );

        try {
          await readLocalLock(dir);
          ...
          expect(lockError.lockVersion).toBe(5);
          expect(lockError.supportedVersion).toBe(1);
          expect(lockError.lockFile).toBe('skills-lock.json');
        }
      } finally {
        await rm(dir, { recursive: true, force: true });
      }
